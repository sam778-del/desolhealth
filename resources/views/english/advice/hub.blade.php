...
@php
    ...
    $advice = \App\Advice::orderBy('id', 'DESC')->get();
@endphp
        ...
        <div class="container">
            <div class="container margin_80_55">
                <div class="main_title_2">
                    ...
                    <h2>Advice Hub</h2>
                    <!-- ================ home Title content  =================== -->
                    <p style="font-size: 15px">Use our list of health topics to reach your destination faster. If there’s something you are looking for which doesn’t fall in to the health topics listed here, use the navigation above.</p>
                </div>
                <div class="row">
                    <!-- ================ news content  =================== -->
                    @foreach($advice as $new)
                        <div class="col-lg-4 col-md-6">
                            <div class="card text-center" style="width: auto; height: auto;">
                                <img src="{{ asset('advice'.'/'.$new->image) }}" height="200" alt="{{ env('APP_NAME') }}">
                                <div class="card-body">
                                    <h5 class="card-title">{{ $new->title, 0, 8 }}</h5>
                                    <p class="card-text">{{ substr($new->description, 0, 80) }}</p>
                                    <a href="{{ route('hub_details', $new->title) }}" class="btn btn-outline-primary">Read More</a>
                                </div>
                            </div>
                        </div>
                    @endforeach
